...
export default function FloorplanEditor() {
  ...
  const findRooms2 = () => {
    ...
    // Queue of corners to be visited
    const queue = [corners.current[0]?.id];

    let prevWall;

    while (queue.length > 0) {
      // Get current corner from the queue
      const currentCornerId = queue.shift();
      console.log(currentCornerId);
      const currentWall = walls.current.find(
        wall => wall.startId === currentCornerId || wall.endId === currentCornerId
      );

      console.log(currentWall);

      // Process the current corner
      console.log("Visiting corner:", currentCornerId);

      // Get adjacent walls of the current corner
      const adjacentWalls = walls.current.filter(
        wall =>
          (wall.startId === currentCornerId || wall.endId === currentCornerId) &&
          wall.id !== prevWall?.id
      );

      console.log(adjacentWalls);

      // Get adjacent corners of current corner
      const adjacentCorners = adjacentWalls.map(wall => {
        if (wall.startId === currentCornerId) return wall.endId;
        if (wall.endId === currentCornerId) return wall.startId;
      });

      console.log(adjacentCorners);

      // Loop through adjacent
      for (const adjCornerId of adjacentCorners) {
        if (!visitedCorners.has(adjCornerId)) {
          queue.push(adjCornerId);
        }
      }
    }
  };
    ...
    for (const corner of corners.current) {
      if (!visitedCorners.has(corner.id)) {
        ...
        while (true) {
          ...
          currentWall = walls.current.find(
            wall =>
          ...
          let nextCorner =
            currentWall.startId === currentCorner ? currentWall.endId : currentWall.startId;

          if (visitedCorners.has(nextCorner)) {
            // Going to a corner we've been at before
            polygons.push(polygonCorners);
            loop = true;
            break;
          }
