...
export const Room = ({ room, walls, mainLoadingManager }) => {
	...
	const finalEdges = useMemo(() => {
		...
		for (let i = 0; i < corners.length; i++) {
			...
			if (wallTo) {
				halfEdges.push({ wall: wallTo, side: "front" });
			} else if (wallFrom) {
				halfEdges.push({ wall: changeDirection(wallFrom), side: "back" });
			} else {
				console.log("Error");
			}
		}
		...
		const finalEdges = halfEdges.map(edge => {
			const { wall, prev, next, side } = edge;
			...
			return { interiorStart, interiorEnd, exteriorStart, exteriorEnd, id, side };
		});
